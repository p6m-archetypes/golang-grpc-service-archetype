...
func Load() (*Config, error) {
	// GRPC_PORT is what the platform manifests inject for gRPC transports.
	port, err := strconv.Atoi(getEnv("GRPC_PORT", "{{ service_port }}"))
	if err != nil {
...
// dbURL returns DATABASE_URL if set, otherwise assembles it from the individual
...
{% elseif persistence == "MySQL" %}
	return "root:root@tcp(localhost:3306)/{{ project-name }}"
{% endif %}}
{% endif %}
{% if has_cache %}

// redisURL returns REDIS_URL if set, otherwise assembles it from the individual
